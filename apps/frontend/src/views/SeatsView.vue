...
onMounted(async () => {
  ...
  connect((event: SeatEvent) => {
    ...
    const seat = showtime.value.seats.find(
      (s) => s.seat_number === event.seat_number,
    );
    if (seat) {
      seat.status = event.status;
      if (
        (event.status === 'locked' || event.status === 'booked') &&
        !isLocked.value
      ) {
        selectedSeats.value = selectedSeats.value.filter(
          (s) => s !== event.seat_number,
        );
      }
    }
  });
...
function getSeatClass(seat: Seat): string {
  if (seat.status === 'locked')
    return 'bg-yellow-500 cursor-not-allowed opacity-70';
  ...
  <div class="min-h-screen bg-gray-900 text-white">
    ...
    <main class="max-w-3xl mx-auto px-6 py-10">
      ...
      <div v-else-if="showtime">
        <!-- Screen indicator -->
        <div
          class="bg-gray-700 text-center text-gray-400 text-sm py-2 rounded-lg mb-8"
        >
          SCREEN
        </div>
        ...
        <div class="flex gap-6 text-sm text-gray-400 mb-8">
          <span class="flex items-center gap-2"
            ><span class="w-4 h-4 rounded bg-gray-600 inline-block"></span>
            Available</span
          >
          <span class="flex items-center gap-2"
